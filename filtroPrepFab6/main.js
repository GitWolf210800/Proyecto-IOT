  ...
  sector.onreadystatechange = () => {
    if (sector.readyState == XMLHttpRequest.DONE) {
      if (sector.status == 200) {
        ...
        document.getElementById(dataFiltroVent).textContent =
          infoFiltroVentilador;
        ...
        ventilador < limitVentilador
          ? (buttonVentilador.style.fill = alertt)
          : (buttonVentilador.style.fill = ok);

        filtroVentilador > limitFiltroVentilador
          ? (buttonFiltroVent.style.fill = alertt)
          : (buttonFiltroVent.style.fill = ok);

        picos < limitPicos
          ? (buttonPicos.style.fill = alertt)
          : (buttonPicos.style.fill = ok);

        rpmFiltro < limitRpmFiltro
          ? (buttonRpm.style.fill = alertt)
          : (buttonRpm.style.fill = ok);

        carro < limitCarro
          ? (buttonCarro.style.fill = alertt)
          : (buttonCarro.style.fill = ok);
...
const actualizarDatos = () => {
  const fab = datosSector(
    ...
    "carroData",
    "carroDataText",
    "http://192.168.3.122:1880/dataFab6prepFiltro"
  );
};
